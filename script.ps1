...
if (-not $DryRun) {
    ...
    if ($branchesToDelete) {
        # ⚡ Bolt Optimization: Batch delete local branches
        # Expected performance impact: Reduces process spawns from O(N) to O(1) by passing all branches to a single git branch -D command
